...
Then use `<script/>` and `<link/>` tags to include to the js and css files in the `static/assets` directory in your HTML. Then include any LUX components in a wrapper with `class="lux"`. (Ignore the `style.css` file for now.)
